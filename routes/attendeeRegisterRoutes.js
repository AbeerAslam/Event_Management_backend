...
{
  const { name, age, education, cnic, phone, email, paymentLocation, eventId } = req.body;

  const attendeeSql = `INSERT INTO attendees (name, age, education, cnic, phone, email, payment_location) VALUES (?, ?, ?, ?, ?, ?, ?)`;
  db.query(attendeeSql, [name, age, education, cnic, phone, email, paymentLocation], (err, attendeeResult) => {
    if (err) {
      console.error(err);
    ...
    const registrationSql = `INSERT INTO registrations (event_id, attendee_id, eligibility_status, registration_status) VALUES (?, ?, 'Pending', 'Pending')`;
...
{
  const query = `SELECT attendees.name, attendees.phone, registrations.* FROM registrations JOIN attendees ON attendees.attendee_id = registrations.attendee_id`; 
