        ...
        db.query(checkResourceQuery, [vendor_id, transport_id, venue_id, event_date], (resourceErr, resourceResult) => {
            ...
            db.query(checkVenueCapacityQuery, [venue_id], (capacityErr, capacityResult) => {
                ...
                const insertEventQuery = `
                    INSERT INTO events (event_name, description, budget, event_date, event_timings, event_category, coordinator_id, vendor_id, transport_id, venue_id, event_request_id)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`;
                db.query(insertEventQuery, [
                    requestResult[0].event_name,description || null,budget || 0,event_date, 
                    event_timings || null,event_category || null,req.user_id,vendor_id,transport_id,venue_id, request_id,],
...
exports.viewApprovedEvents = (req, res) => {
    ...
    db.query(viewEventsQuery, (err, results) => {
        if (err) {
            return res.status(500).json({ error: 'Error fetching events for attendees: ' + err.message });
        }
